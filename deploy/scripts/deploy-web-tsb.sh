...
git clone -b master dreamix-git:tsb-passport
git clone -b TSB-release dreamix-git:tsb-web
git clone -b master dreamix-git:cola && git clone -b pre-release dreamix-git:tsb-server
...
sed -Ei "/mongo/{n;n;n;n;s/'login'.+?/'login' => '${MONGO_USERNAME}',/}" /data/www/htdocs/tsb-server/app/config/database.php
sed -Ei "/mongo/{n;n;n;n;n;s/'password'.+?/'password' => '${MONGO_PASSWORD}',/}" /data/www/htdocs/tsb-server/app/config/database.php
sed -Ei "/mongo/{n;n;n;n;n;n;s/'database'.+?/'database' => '${MONGO_DATABASE}',/}" /data/www/htdocs/tsb-server/app/config/database.php
